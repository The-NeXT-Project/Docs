...
This is the monthly free allowance. Both must be non-zero for anything to happen. Per-account overrides live on the user edit page.
...
`Settings` → `Other settings` is the remainder: what users can see, what gets logged, and check-in.
...
Record sign-ins | On | —
Notify the user of a sign-in from a new IP | Off | —
...
Covered in [Shop](shop.md#automatic-tidying).
